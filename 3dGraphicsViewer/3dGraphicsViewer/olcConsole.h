...
public:
	void Start()
	{
		// Start the thread
		m_bAtomActive = true;
		std::thread t = std::thread(&olcConsole::GameThread, this);
...
protected: // Audio Engine =====================================================================
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_AudioThread = std::thread(&olcConsole::AudioThread, this);
	...
	{
		((olcConsole*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
...
std::atomic<bool> olcConsole::m_bAtomActive(false);
std::condition_variable olcConsole::m_cvGameFinished;
std::mutex olcConsole::m_muxGame;